...
const ScorePerCampusChart = ({ setScores }) => {
        ...
        const getFormulasForAnswers = async () => {
            try {
                const answerResponse = await fetch(
                    `http://localhost:9000/api/get/records-values-by-sdg_id/`
                );
                ...
                console.log(answerData, "lala");
                const new_anwers = answerData.filter(
                    (answer) => answer.sdg_id === selectedSdg
                );

                setAnswers(new_anwers); // Filter answers by selected SDG

                let formulasArray = [];
                for (const answer of new_anwers) {
    ...
    const calculatedCampuses = useMemo(() => {
        if (
            ...
            return campuses; // Return campuses without modifying if data is incomplete
        }
        ...
        return campuses.map((campus) => {
            ...
            formulas.forEach((formula) => {
                let valueMap = {}; // Map to hold sub_id -> value from answers
    ...
    }, [formulas, answers, selectedSdg]); // Only recalculate when these dependencies change

    // Update the scores state with the calculated total scores
    useEffect(() => {
        setCampuses(calculatedCampuses);
        setScores(calculatedCampuses);
